...
相比于 `TCP`，`UDP` 是面向无连接的，也就是说在正式传递数据之前不需要先建立连接。`UDP` 协议不保证有序且不丢失的传递到对端，也就是说不够稳定，在网络不好的情况下很容易出现丢包的情况。但是它也有自己的优势，如果是对于实时性要求比较高的应用，比如直播、LOL 等对战游戏，则会基于`UDP` 协议实现。
